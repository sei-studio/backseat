 ...
 * That split matters: the four wakes fire at wildly different rates and
 ...
 *
 * Silence is NOT an outcome the model chooses (260802). Every look that
 * reaches the model produces a line; the only silence left is the mechanical
 * MIN_SPEAK_GAP_MS drop above, made before the model is ever called. A
 * "(silence)" reply still parses, because one must never be spoken aloud, but
 * it is logged as an anomaly rather than expected. See runTurn.
 */
